...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(GEN_OUT_MULT, GEN_OUT_MULT, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(GEN_OUT_MULT),
            nn.LeakyReLU(negative_slope= 0.05, inplace = True),
            ...
            nn.ConvTranspose2d(GEN_OUT_MULT * 2, GEN_OUT_MULT * 2, kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(GEN_OUT_MULT * 2),
            nn.LeakyReLU(negative_slope= 0.05, inplace = True),
            
            nn.ConvTranspose2d(GEN_OUT_MULT * 2, IMAGE_CHANNELS, kernel_size=4, stride=2, padding=1, bias=False),                     
            nn.Tanh() 
        )
